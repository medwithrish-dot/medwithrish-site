...
import { parseDoneReply, ANSWER_SILENCE_MS, DONE_PROMPT, questionTransition } from "../app/medicforest/interview/_lib/station-flow.ts";
...
test("station timing and lobby labels do not preview the hidden topic", () => {
  ...
  const lobby = readFileSync(resolve(root, "app/medicforest/interview/_components/AIInterviewSetup.tsx"), "utf8");
...
async function autosaveRoom({ status = "in_progress", preparationSeconds = 0, hasAttempt = true, configured = true, followUps = false, permission = true, speechError = "", voiceSupported = false, delayedVoice = false, questions = ["Why medicine?"], initialDraft } = {}) {
  ...
  if (initialDraft) storage.set(`medicforest-interview-draft:${circuitId}`, JSON.stringify(initialDraft));
  ...
  const source = readFileSync(resolve(root, "app/medicforest/interview/_components/AIInterviewRunner.tsx"), "utf8");
...
test("finishing empty or short answers opens a saved review without generating feedback", async () => {
  for (const answer of ["", "I enjoy caring for others."]) {
    ...
    assert.equal(room.storage.has(`medicforest-interview-draft:${circuitId}`), false, "Remove the browser draft only after the server confirms saving");
    assert.equal(room.feedbackRequests.length, 0);
  }
});
...
test("a failed finish keeps the transcript in review and retains the recoverable browser draft", async () => {
  ...
  assert.equal(review.busy, false);
  const draft = JSON.parse(room.storage.get(`medicforest-interview-draft:${circuitId}`));
...
function microphoneDevices({ supported = true } = {}) {
  ...
  const loaded = { exports: {} };
  runInNewContext(ts.transpileModule(readFileSync(resolve(root, "app/medicforest/interview/_lib/useInterviewDevices.ts"), "utf8"), {
