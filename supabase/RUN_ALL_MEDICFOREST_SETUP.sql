-- COMPLETE MEDICFOREST CATCH-UP SETUP
-- Run this entire file once in Supabase SQL Editor using the postgres role.
-- Includes original accounts, UCAT practice, billing and every interview migration.
-- Can be rerun; existing account plans, used credits and practice history are preserved.
-- The legacy practice credit reset is deliberately omitted.
-- Does not configure hosting secrets, Stripe products/webhooks or Auth settings.

begin;

-- STEP 1 OF 8: medicforest_setup.sql

-- MedicForest / UCAT Tutor Supabase setup
...
-- STEP 2 OF 8: medicforest_practice_setup.sql

-- MedicForest practice telemetry setup
...
-- Preserve existing diagnostic credits when catching up or rerunning.
...
-- STEP 3 OF 8: medicforest_stripe_setup.sql
...
-- STEP 4 OF 8: medicforest_security_patch.sql
...
-- STEP 5 OF 8: medicforest_interview_question_progress.sql
...
-- STEP 6 OF 8: medicforest_interview_platform.sql
...
-- STEP 7 OF 8: medicforest_interview_groups.sql
...
-- STEP 8 OF 8: medicforest_interview_dashboard.sql
...
-- Create missing profiles for accounts registered before the profile trigger existed.
insert into public.profiles (id, email, full_name)
