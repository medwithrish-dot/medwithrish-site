...
function getBackHref(searchParams: QuestionBankSectionSearchParams) {
  ...
  if (diagnosticMode === "subset") {
    return withMockQuery("/medicforest/ucat/question-bank?diagnostic=subset", mockId);
  }

  if (diagnosticMode === "full-section") {
    return withMockQuery("/medicforest/ucat/mocks/full", mockId);
  }
    ...
    if (!isPremium) {
      return (
        <PremiumDiagnosticLock
          backHref={backHref ?? "/medicforest/ucat/diagnostic"}
