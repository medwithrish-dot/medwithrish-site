...
const MOCK_SESSION_DRAFT_PREFIX = "medicforest-mock-draft:v1:";
const MOCK_SESSION_DRAFT_EVENT = "medicforest-mock-drafts-changed";
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-8 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <h1 className="mt-3 text-3xl font-black text-slate-950">
            Choose a MedicForest section to practise
          </h1>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-4">
            {UCAT_SECTIONS.map((section) => (
              <Link
                key={section.slug}
                href={`/medicforest/ucat/question-bank/${section.slug}`}
...
function SectionSetup({
  ...
  return (
    <div className="min-h-screen bg-[#f6f8fb] px-4 py-8 text-[#111827]">
      <div className="mx-auto max-w-5xl">
        <Link
          href={diagnosticMode ? backHref ?? "/medicforest/ucat/diagnostic" : "/medicforest/ucat/practice"}
          className="inline-flex items-center gap-2 text-sm font-bold text-blue-700 hover:text-blue-900"
        >
          <ArrowLeft className="h-4 w-4" aria-hidden="true" />
        ...
        <section className="mt-8 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs font-semibold leading-5 text-amber-900">
            ...
            <p className="mt-2">
              When you mark a set, MedicForest can save answers, timings, answer
              switches, flags, calculator usage, keyboard shortcuts, navigation
              activity and overall results for feedback. AI feedback is educational
              support only and does not guarantee exam or admissions outcomes.
            </p>
            <div className="mt-2 flex flex-wrap gap-3 font-black">
              <Link href="/privacy-policy" className="text-blue-700 underline">
                Privacy Policy
              </Link>
              <Link href="/medicforest-disclaimer" className="text-blue-700 underline">
                AI/Data Disclaimer
              </Link>
            </div>
          </div>
          ...
          <button
            type="button"
            onClick={onStart}
            disabled={progressLoading || questionCount === 0}
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-7 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-slate-600">
              MedicForest now builds mocks from random uncompleted questions in the
          ...
          <div className="mt-7">
            <MockSetPicker
              selectedMockId={mockId}
              baseHref="/medicforest/ucat/mocks/full"
            />
          </div>
            ...
            {[
              {
                ...
                href: withMockQuery("/medicforest/ucat/mocks/full", mockId),
              ...
              },
              {
                ...
                href: withMockQuery("/medicforest/ucat/question-bank?diagnostic=subset", mockId),
...
function MockStartPanel({
  ...
  const actionHref = mockComplete
    ? `/medicforest/ucat/report?mock=${selectedMock.id}`
    : withMockQuery(`/medicforest/ucat/mocks/full/${nextSection}`, selectedMock.id);
      ...
      <main className="mx-auto max-w-6xl rounded-xl border border-slate-200 bg-white px-5 py-5 shadow-sm">
              ...
              {[
                {
                  ...
                  href: withMockQuery(`/medicforest/ucat/mocks/full/${nextFullMockSection}`, selectedMock.id),
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {sectionPickerOptions.map(({ section, code, title }) => (
                <Link
                  key={section}
                  href={withMockQuery(
                    `/medicforest/ucat/mocks/subtest/${section}`,
                    selectedMock.id
                  )}
                  className="flex flex-col rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition-colors hover:border-blue-400 hover:bg-blue-50/60"
                >
        ...
        <div className="mt-3 space-y-2.5">
          {visibleMocks.length === 0 ? (
            ...
            const actionHref = mockComplete
              ? `/medicforest/ucat/report?mock=${mock.id}`
              : withMockQuery(`/medicforest/ucat/mocks/full/${nextSection}`, mock.id);
            const actionLabel = mockComplete
              ? "Open diagnostic report"
              : mockStarted || active
                ? "Continue"
                  ...
                  <div className="mt-2.5 flex flex-wrap gap-1.5">
                    {FULL_MOCK_SECTION_ORDER.map((section) => {
                      ...
                      const sectionHref = score?.sessionId
                        ? withMockQuery(
                            `/medicforest/ucat/mocks/full/${section}?set=${encodeURIComponent(
                              score.sessionId
                            )}`,
                            mock.id
                          )
                        : withMockQuery(
                            `/medicforest/ucat/mocks/full/${section}`,
                            mock.id
                          );
                      return (
                        <Link
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-7 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6">
            <MockSetPicker
              selectedMockId={mockId}
              baseHref="/medicforest/ucat/question-bank?diagnostic=subset"
              compact
            />
          </div>
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-2">
            {UCAT_SECTIONS.map((section) => {
              const href = `/medicforest/ucat/question-bank/${section.slug}?diagnostic=${
...
function MarkedReviewScreen({
  ...
  const diagnosticReportHref = attemptId
    ? `/medicforest/ucat/report?attempt=${encodeURIComponent(attemptId)}`
...
function UCATQuestionBankSection({
  ...
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      const state = event.state as
        | { medicforestQuestionBankView?: string }
        | null;

      if (
        state?.medicforestQuestionBankView === "marked-insights" &&
  ...
  const showMarkedInsights = () => {
    if (
      ...
      window.history.pushState(
        { medicforestQuestionBankView: "marked-insights" },
        "",
  ...
  if (phase === "diagnostic-complete" && markedSummary) {
    ...
    const nextSectionHref = nextMockSection
      ? withMockQuery(`/medicforest/ucat/mocks/full/${nextMockSection}`, mockId)
      : withMockQuery("/medicforest/ucat/mocks/full", mockId);
            ...
            >
              ...
              <div className="mt-2 grid grid-cols-[78px_minmax(0,1fr)] items-end gap-2">
                ...
                <div className="pb-0.5 text-right text-white">
                  <div className="text-[10px] font-bold uppercase">MEDICFOREST</div>
