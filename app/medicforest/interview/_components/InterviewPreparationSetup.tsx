...
export function InterviewPreparationSetup({ initialProfile, signedIn, available, variant = "full" }: InterviewPreparationSetupProps) {
  ...
  return <section id="preparation-setup" className={`overflow-hidden rounded-2xl border shadow-sm ${savedProfile ? "border-[#d5e2e1] bg-white" : "border-[#c1dcd5] bg-[#f8fcfa]"}`} aria-labelledby={`${uniqueId}-title`}>
    ...
    {error && <div role="alert" className="mx-5 mb-4 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm leading-6 text-red-800 sm:mx-6">{error}{sessionExpired && <p className="mt-2"><Link href="/medicforest/account" className="font-bold underline">Sign in again</Link>, then return to finish setting up your plan.</p>}</div>}
    ...
    </form></div>{variant === "compact" && <div className="flex items-center justify-between gap-4 border-t border-[#d7e3e1] bg-[#f7faf9] px-5 py-3"><p className="text-[11px] leading-5 text-[#637b78]">{showFullEditor ? "The complete form is open." : "Scroll inside the form for university choices, entry route and experience."}</p><button type="button" onClick={() => setShowFullEditor((current) => !current)} className="inline-flex shrink-0 items-center gap-2 rounded-lg border border-[#cdded8] bg-white px-3 py-2 text-xs font-bold text-[#08787b] hover:bg-[#edf7f3]" aria-expanded={showFullEditor}>{showFullEditor ? "Use compact view" : "Expand full form"}<ChevronDown size={14} className={`transition-transform ${showFullEditor ? "rotate-180" : ""}`} aria-hidden="true" /></button></div>}</div></div>}
  </section>;
}
