        ...
        <div className={`${styles.conversationStage} ${stimulus ? styles.presentationStage : ""}`}>
          {stimulus ? <InterviewStimulus key={stimulus.id} stimulus={stimulus} question={question} presentation slideLabel={`Question ${questionIndex + 1} of ${attempt.questions.length}`} /> : <div className={styles.participantGrid}>
            ...
            <section className={`${styles.participantCard} ${styles.aiParticipant} ${speech.speaking ? styles.participantActive : ""}`} aria-label="AI interviewer participant tile">
              ...
              <div className={styles.participantName}><div><strong>AI interviewer</strong><span>{speech.speaking ? "Speaking" : "MedicForest"}</span></div>{speech.speaking ? <AudioLines size={15} aria-label="Reading the question" /> : <span className={styles.greenDot} />}</div>
            </section>
          </div>}
