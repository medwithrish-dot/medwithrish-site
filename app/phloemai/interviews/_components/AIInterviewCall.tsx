...
import { ArrowLeft, ArrowRight, AudioLines, Check, Clock3, FileText, Loader2, LockKeyhole, Maximize2, MessageSquareText, Mic, MicOff, Minimize2, PhoneOff, Sparkles, UserRound, Users, Video, VideoOff, Volume2, VolumeX, X } from "lucide-react";
  ...
  return <div className={`${styles.callRoom} ${focus ? styles.focusRoom : ""}`}>
    <header className={styles.roomHeader}>
      ...
      <div className={styles.headerActions}>
        ...
        <span className={styles.participantCount} aria-label="Two participants: you and the AI interviewer"><Users size={18} />2</span>
        ...
        <div className={styles.conversationStage}>
          <div className={styles.participantGrid}>
            <section className={`${styles.participantCard} ${speech.listening ? styles.participantActive : ""}`} aria-label="Your participant tile">
              {devices.cameraEnabled ? <div className={styles.participantVideo}><InterviewDevicePreview stream={devices.stream} compact /></div> : <div className={styles.participantPortrait}><span className={styles.youAvatar} aria-hidden="true">Y</span></div>}
              <div className={styles.participantName}><strong>You</strong>{speech.listening && <AudioLines size={16} aria-label="Microphone on" />}</div>
            </section>
            <section className={`${styles.participantCard} ${styles.aiParticipant} ${speech.speaking ? styles.participantActive : ""}`} aria-label="AI interviewer participant tile">
              <div className={styles.participantPortrait}><div className={`${styles.interviewerOrb} ${speech.speaking ? styles.orbSpeaking : ""}`} aria-hidden="true"><span /><div>AI</div></div></div>
              <div className={styles.participantName}><strong>AI Interviewer</strong>{speech.speaking && <AudioLines size={16} aria-label="Reading the question" />}</div>
            </section>
          </div>
