...
import { ArrowLeft, ArrowRight, AudioLines, Check, Clock3, FileText, Leaf, Loader2, LockKeyhole, Maximize2, MessageSquareText, Mic, MicOff, Minimize2, PhoneOff, Sparkles, UserRound, Video, VideoOff, Volume2, VolumeX, X } from "lucide-react";
  ...
  return <div className={`${styles.callRoom} ${focus ? styles.focusRoom : ""}`}>
    <header className={styles.roomHeader}>
      <div className={styles.roomBrand}>
        <span className={styles.brandMark}><Leaf size={22} strokeWidth={1.6} /></span>
        <div><p>PHLOEMAI <span>/</span> INTERVIEW ROOM</p><h1>{attempt.title}</h1></div>
      </div>
    ...
    <div className={styles.callLayout}>
      <div className={styles.stageColumn}>
        <div className={styles.stageTop}><span className={styles.roomPill}>Station {attempt.stationIndex + 1}<span>/ {attempt.stationCount}</span></span><span className={styles.stageStatus} role="status"><span className={speech.listening ? styles.listeningDot : styles.greenDot} />{status}</span></div>
        <div className={styles.conversationStage}>
          <div className={styles.participantGrid}>
            <section className={`${styles.participantCard} ${speech.listening ? styles.participantActive : ""}`} aria-label="Your participant tile">
              <div className={styles.participantTop}><span>YOU</span>{speech.listening ? <AudioLines size={17} /> : <MicOff size={16} />}</div>
              {devices.cameraEnabled ? <div className={styles.participantVideo}><InterviewDevicePreview stream={devices.stream} compact /></div> : <div className={styles.participantPortrait}><span className={styles.youAvatar}><UserRound size={42} strokeWidth={1.25} /></span></div>}
              <div className={styles.participantName}><strong>You</strong><span>{speech.listening ? "Microphone on" : devices.cameraEnabled ? "Camera only visible to you" : "Camera off · your space to practise"}</span></div>
            </section>
            <section className={`${styles.participantCard} ${styles.aiParticipant} ${speech.speaking ? styles.participantActive : ""}`} aria-label="AI interviewer participant tile">
              <div className={styles.participantTop}><span>PHLOEMAI</span><Sparkles size={16} /></div>
              <div className={styles.participantPortrait}><div className={`${styles.interviewerOrb} ${speech.speaking ? styles.orbSpeaking : ""}`} aria-hidden="true"><span /><span /><div><AudioLines size={38} strokeWidth={1.4} /></div></div></div>
              <div className={styles.participantName}><strong>Your interviewer</strong><span>{speech.speaking ? "Reading your question" : speech.listening ? "Listening to your answer" : "Here to help you prepare"}</span></div>
            </section>
          </div>
          <div className={styles.questionCaption}>
            <div className={styles.promptHeading}><span>YOUR PROMPT <span>{String(questionIndex + 1).padStart(2, "0")} / {String(attempt.questions.length).padStart(2, "0")}</span></span><button type="button" onClick={() => speech.speaking ? speech.stopSpeaking() : void speech.speak(question)} disabled={!speech.voiceSupported || Boolean(busy)}>{speech.speaking ? <VolumeX size={15} /> : <Volume2 size={15} />}{speech.speaking ? "Stop reading" : "Hear question"}</button></div>
            <p id="current-interview-question">{question}</p>
          ...
          <div className={styles.stationHint}><p>{preparing ? "A moment to gather your thoughts. Your answer opens after reading time." : "Take your time. Bring your experiences into the conversation."}</p>{preparing && preview && <button type="button" className={styles.textButton} onClick={props.onSkipPreparation}>Skip reading time <ArrowRight size={14} /></button>}</div>
        </div>
        <footer className={styles.callControls} aria-label="Interview controls">
          <div className={styles.deviceControls}>
            ...
            <button type="button" aria-label={devices.cameraEnabled ? "Turn camera off" : "Turn camera on"} title={devices.cameraEnabled ? "Turn camera off" : "Turn camera on"} aria-pressed={devices.cameraEnabled} disabled={devices.cameraPending || Boolean(busy)} onClick={() => void devices.toggleCamera()}>{devices.cameraPending ? <Loader2 size={21} className="animate-spin" /> : devices.cameraEnabled ? <Video size={21} /> : <VideoOff size={21} />}<span>Camera</span></button>
            <button type="button" aria-label={props.readAloud ? "Turn interviewer voice off" : "Turn interviewer voice on"} title={props.readAloud ? "Turn interviewer voice off" : "Turn interviewer voice on"} aria-pressed={props.readAloud} disabled={!speech.voiceSupported} onClick={() => props.setReadAloud(!props.readAloud)}>{props.readAloud ? <Volume2 size={21} /> : <VolumeX size={21} />}<span>Voice</span></button>
      ...
      <aside className={styles.transcriptPanel} aria-label="Interview transcript and notes">
        <div className={styles.transcriptTabs}><button type="button" aria-pressed={tab === "transcript"} onClick={() => setTab("transcript")}><MessageSquareText size={17} />Transcript<span className={styles.transcriptCount}>{questionIndex + 1}</span></button><button type="button" aria-pressed={tab === "notes"} onClick={() => setTab("notes")}><FileText size={17} />Notes{notes && <span className={styles.notesDot} aria-label="Notes added" />}</button></div>
        {tab === "transcript" ? <>
          <div className={styles.transcriptScroll}>
            <div className={styles.transcriptIntro}><span><span className={styles.greenDot} />LIVE TRANSCRIPT</span><span>Question {questionIndex + 1} of {attempt.questions.length}</span></div>
            <div className={styles.transcriptQuestion}><span className={styles.miniInterviewer}><AudioLines size={18} /></span><div><strong>PhloemAI interviewer</strong><p>{question}</p></div></div>
            ...
            <p className={styles.characterCount}>{answer.length.toLocaleString()} / 6,000 characters</p>
