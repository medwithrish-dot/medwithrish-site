...
export function SavedInterviewReview({ initialAttempt, configured: initialConfigured, serverNow, abandoned = false, hasLaterStation = false }: {
  ...
  const interviewHref = `/phloemai/interviews/ai-interviews?attempt=${encodeURIComponent(attempt.id)}`;
  ...
  const runAction = useCallback(async (action: Action) => {
    ...
    try {
      ...
      if (action === "generate") {
        ...
        router.push(`/phloemai/interviews/ai-interviews?attempt=${encodeURIComponent(response.attempt.id)}`);
      }
    } catch (failure) {
      if (liveRef.current) {
  ...
  return <>
    {error && <div role="alert" className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">
      ...
      {errorStatus === 401 && <Link href="/phloemai/account" className="mt-2 inline-block font-bold underline">Sign in to your account</Link>}
      {errorStatus === 403 && <Link href="/phloemai/pricing" className="mt-2 inline-block font-bold underline">View membership options</Link>}
    </div>}
    {notice && <div role="status" className="mb-5 rounded-xl border border-[#cce2db] bg-[#edf7f2] p-4 text-sm leading-6 text-[#254f46]"><p>{notice}</p>{activeAttemptId && <Link href={`/phloemai/interviews/ai-interviews?attempt=${encodeURIComponent(activeAttemptId)}`} className="mt-2 inline-flex items-center gap-2 font-bold underline">Resume active interview <ArrowRight size={15} /></Link>}</div>}
