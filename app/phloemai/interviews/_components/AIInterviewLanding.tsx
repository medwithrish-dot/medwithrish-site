...
export async function AIInterviewLanding() {
  ...
  for (const attempt of completed) {
    ...
    recentUniversityAttempts[attempt.universitySlug] = {
      href: `/phloemai/interviews/reports/${encodeURIComponent(attempt.id)}`,
    ...
    <div className={styles.landing}>
      <section className={styles.hero} aria-labelledby="practice-title">
        ...
        {freeAttempt ? <div className={styles.heroActions}>
          <Link href={`/phloemai/interviews/reports/${encodeURIComponent(freeAttempt.id)}`} className={styles.primaryAction}><BookOpenCheck size={18} aria-hidden="true" /> Review last attempt <ArrowRight size={18} aria-hidden="true" /></Link>
          <Link href="/phloemai/interviews/ai-interviews?station=why-medicine" className={styles.retryAction}><RotateCcw size={15} aria-hidden="true" /> Retry the challenge</Link>
        </div> : <Link href="/phloemai/interviews/ai-interviews?station=why-medicine" className={styles.primaryAction}><Mic size={18} aria-hidden="true" /> Try the free interview <ArrowRight size={18} aria-hidden="true" /></Link>}
      </section>
