...
function getBackHref(searchParams: QuestionBankSectionSearchParams) {
  ...
  if (diagnosticMode === "subset") {
    return withMockQuery("/phloemai/ucat/question-bank?diagnostic=subset", mockId);
  }

  if (diagnosticMode === "full-section") {
    return withMockQuery("/phloemai/ucat/mocks/full", mockId);
  }
    ...
    if (!isPremium) {
      return (
        <PremiumDiagnosticLock
          backHref={backHref ?? "/phloemai/ucat/diagnostic"}
