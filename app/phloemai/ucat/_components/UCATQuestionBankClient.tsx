        ...
        <div className="mt-8 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-4">
            {UCAT_SECTIONS.map((section) => (
              <Link
                key={section.slug}
                href={`/phloemai/ucat/question-bank/${section.slug}`}
...
function SectionSetup({
  ...
  return (
    <div className="min-h-screen bg-[#f6f8fb] px-4 py-8 text-[#111827]">
      <div className="mx-auto max-w-5xl">
        <Link
          href={diagnosticMode ? backHref ?? "/phloemai/ucat/diagnostic" : "/phloemai/ucat/practice"}
          className="inline-flex items-center gap-2 text-sm font-bold text-blue-700 hover:text-blue-900"
        >
          <ArrowLeft className="h-4 w-4" aria-hidden="true" />
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-7 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-7">
            <MockSetPicker
              selectedMockId={mockId}
              baseHref="/phloemai/ucat/mocks/full"
            />
          </div>
            ...
            {[
              {
                ...
                href: withMockQuery("/phloemai/ucat/mocks/full", mockId),
              ...
              },
              {
                ...
                href: withMockQuery("/phloemai/ucat/question-bank?diagnostic=subset", mockId),
...
function MockStartPanel({
  ...
  const actionHref = mockComplete
    ? `/phloemai/ucat/report?mock=${selectedMock.id}`
    : withMockQuery(`/phloemai/ucat/mocks/full/${nextSection}`, selectedMock.id);
      ...
      <main className="mx-auto max-w-6xl rounded-xl border border-slate-200 bg-white px-5 py-5 shadow-sm">
              ...
              {[
                {
                  ...
                  href: withMockQuery(`/phloemai/ucat/mocks/full/${nextFullMockSection}`, selectedMock.id),
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {sectionPickerOptions.map(({ section, code, title }) => (
                <Link
                  key={section}
                  href={withMockQuery(
                    `/phloemai/ucat/mocks/subtest/${section}`,
                    selectedMock.id
                  )}
                  className="flex flex-col rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition-colors hover:border-blue-400 hover:bg-blue-50/60"
                >
        ...
        <div className="mt-3 space-y-2.5">
          {visibleMocks.length === 0 ? (
            ...
            const actionHref = mockComplete
              ? `/phloemai/ucat/report?mock=${mock.id}`
              : withMockQuery(`/phloemai/ucat/mocks/full/${nextSection}`, mock.id);
            const actionLabel = mockComplete
              ? "Open diagnostic report"
              : mockStarted || active
                ? "Continue"
                  ...
                  <div className="mt-2.5 flex flex-wrap gap-1.5">
                    {FULL_MOCK_SECTION_ORDER.map((section) => {
                      ...
                      const sectionHref = score?.sessionId
                        ? withMockQuery(
                            `/phloemai/ucat/mocks/full/${section}?set=${encodeURIComponent(
                              score.sessionId
                            )}`,
                            mock.id
                          )
                        : withMockQuery(
                            `/phloemai/ucat/mocks/full/${section}`,
                            mock.id
                          );
                      return (
                        <Link
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-7 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6">
            <MockSetPicker
              selectedMockId={mockId}
              baseHref="/phloemai/ucat/question-bank?diagnostic=subset"
              compact
            />
          </div>
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-2">
            {UCAT_SECTIONS.map((section) => {
              const href = `/phloemai/ucat/question-bank/${section.slug}?diagnostic=${
...
function FixedDiagnosticStartScreen({
  ...
  onTrackingModeChange,
  onTrackingRingChange,
  backHref = "/phloemai/ucat/diagnostic",
...
function MarkedReviewScreen({
  ...
  const diagnosticReportHref = attemptId
    ? `/phloemai/ucat/report?attempt=${encodeURIComponent(attemptId)}`
...
function UCATQuestionBankSection({
  ...
  if (phase === "diagnostic-complete" && markedSummary) {
    ...
    const nextSectionHref = nextMockSection
      ? withMockQuery(`/phloemai/ucat/mocks/full/${nextMockSection}`, mockId)
      : withMockQuery("/phloemai/ucat/mocks/full", mockId);
