  ...
  return INTERVIEW_PATHWAY.map((station) => ({
    ...
    tasks: [
      ...station.guides.map((slug) => {
        ...
        return { id: pathwayTaskId(station.id, "guide", slug), kind: "guide" as const, title: guide.title, href: `/phloemai/interview/guides/${slug}` };
      }),
      ...station.questions.map((id) => {
        ...
        return { id: pathwayTaskId(station.id, "question", id), kind: "question" as const, title: question.text, href: `/phloemai/interview/question-bank?question=${encodeURIComponent(id)}` };
      }),
    ],
  }));
}
