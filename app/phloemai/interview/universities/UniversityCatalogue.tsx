          ...
          {visible.map((entry) => {
            ...
            return <article key={entry.slug} className={styles.card}>
              <div className={styles.cardHeader}>
                ...
                <div><h3><Link href={`/phloemai/interview/universities/${entry.slug}`}>{entry.name}</Link></h3><span className={styles.format}>{entry.format === "Unconfirmed" ? "General practice" : entry.format === "Mixed" ? "Mixed / group" : entry.format}</span></div>
              </div>
              ...
              <div className={styles.cardFooter}>
                {recentAttempt ? <div className={styles.cardActions}>
                  ...
                  <Link href={`/phloemai/interview/ai-interviews?university=${entry.slug}`} className={styles.cardRetryAction} aria-label={`Retry AI interview for ${entry.name}`}><RotateCcw size={14} aria-hidden="true" />Retry interview</Link>
                </div> : <Link href={`/phloemai/interview/ai-interviews?university=${entry.slug}`} className={styles.primaryAction} aria-label={`Start AI interview for ${entry.name}`}><Mic size={16} aria-hidden="true" />Start AI interview<ArrowRight size={16} aria-hidden="true" /></Link>}
                <div className={styles.details}><span>{recentAttempt ? `Last attempt ${new Date(recentAttempt.completedAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", timeZone: "Europe/London" })}` : entry.timingStatus === "published" ? "Reported timing" : "Practice preset"}</span><Link href={`/phloemai/interview/universities/${entry.slug}`} aria-label={`View format and sources for ${entry.name}`}>Format &amp; sources</Link></div>
              </div>
            </article>;
          })}
        </div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {visible.map((entry) => (
            <article key={entry.slug} className="flex flex-col rounded-2xl border border-[#d8e0e6] bg-white p-6 shadow-sm transition-shadow hover:shadow-md">
              ...
              <h2 className="mt-4 text-lg font-bold leading-6 text-[#042724]"><Link href={`/phloemai/interview/universities/${entry.slug}`} className="hover:text-[#08787b]">{entry.name}</Link></h2>
              ...
              <div className="mt-5 flex flex-wrap items-center gap-4 border-t border-[#edf1f2] pt-4">
                {practiceMode ? (
                  <>
                    <Link href={`/phloemai/interview/ai-interviews?university=${entry.slug}`} className="inline-flex items-center gap-2 rounded-lg bg-[#08787b] px-3 py-2 text-sm font-bold text-white hover:bg-[#065f61]">Start AI interview <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link>
                    <Link href={`/phloemai/interview/universities/${entry.slug}`} className="text-xs font-bold text-[#08787b]">Format &amp; sources</Link>
                  </>
                ) : (
                  <>
                    <Link href={`/phloemai/interview/universities/${entry.slug}`} className="inline-flex items-center gap-2 rounded-lg bg-[#08787b] px-3 py-2 text-sm font-bold text-white hover:bg-[#065f61]">View details <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link>
                    <Link href={`/phloemai/interview/ai-interviews?university=${entry.slug}`} className="text-xs font-bold text-[#08787b]">Practise</Link>
                  </>
                )}
              </div>
            </article>
