...
export function InterviewQuestionProgressMini({
  ...
  return <section className="rounded-2xl border border-[#d7e3e1] bg-white p-5" aria-labelledby="question-progress-title">
    <div className="flex items-center justify-between gap-3">
      ...
      <Link href="/phloemai/interview/question-bank" className="text-[10px] font-bold text-[#08787b]">Open bank</Link>
    </div>
    ...
    <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3">
      {progress.categories.map((category, index) => <Link
        ...
        href={`/phloemai/interview/question-bank?category=${encodeURIComponent(category.category)}`}
    ...
    <Link href="/phloemai/interview/question-bank" className="mt-4 inline-flex items-center gap-1 text-[10px] font-bold text-[#08787b]">Continue questions <ArrowRight className="h-3 w-3" /></Link>
  </section>;
}
