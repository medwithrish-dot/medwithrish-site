    ...
    <div className="space-y-8">
      ...
      {!filtering && <section aria-labelledby="featured-guides-heading">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {interviewGuides.filter((item) => item.featured).map((item) => (
            <Link key={item.slug} href={`/phloemai/interview/guides/${item.slug}`} className="group flex flex-col rounded-2xl border border-[#cfe0d9] bg-[#f3f8f2] p-5 transition-colors hover:border-[#74a49c] hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#08787b]">
      ...
      <section aria-labelledby="all-guides-heading">
        ...
        {results.length ? <div className="grid gap-3 lg:grid-cols-2">
          {results.map((item) => (
            <Link key={item.slug} href={`/phloemai/interview/guides/${item.slug}`} className="group flex items-center justify-between gap-4 rounded-xl border border-[#dce6e5] bg-white p-5 transition-colors hover:border-[#8bb8b1] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#08787b]">
