...
export function SavedInterviewReview({ initialAttempt, configured: initialConfigured, serverNow, abandoned = false, hasLaterStation = false }: {
  ...
  const interviewHref = `/phloemai/interview/ai-interviews?attempt=${encodeURIComponent(attempt.id)}`;
  ...
  const runAction = useCallback(async (action: Action) => {
    ...
    try {
      ...
      if (action === "generate") {
        ...
        router.push(`/phloemai/interview/ai-interviews?attempt=${encodeURIComponent(response.attempt.id)}`);
      }
    } catch (failure) {
      if (liveRef.current) {
  ...
  return <>
    ...
    {notice && <div role="status" className="mb-5 rounded-xl border border-[#cce2db] bg-[#edf7f2] p-4 text-sm leading-6 text-[#254f46]"><p>{notice}</p>{activeAttemptId && <Link href={`/phloemai/interview/ai-interviews?attempt=${encodeURIComponent(activeAttemptId)}`} className="mt-2 inline-flex items-center gap-2 font-bold underline">Resume active interview <ArrowRight size={15} /></Link>}</div>}
