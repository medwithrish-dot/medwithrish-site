...
- **Interview:** the active workspace fills the viewport, including navigation, notices and the room header. A prominent question card sits above compact participant tiles on desktop, retaining the restrained slate/teal palette. The narrower navigation sidebar, transcript and Notes scroll internally; question navigation and labelled microphone, camera and finish controls stay visible. Small screens keep a compact participant strip above the transcript, with controls in a bottom row; short screens prioritise the answer editor. Voice and focus controls remain in the header. Escape closes the finish dialog before leaving focus view. Existing server timers, draft recovery, autosave and scoring are retained.
