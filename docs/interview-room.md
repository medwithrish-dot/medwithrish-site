...
- **Follow-ups:** after at least 20 words, choose **Follow up on my answer** to save the answer and ask one deeper question. Each original prompt permits one follow-up. The server inserts it into the saved station, and its answer is included in feedback and reports. Typed drafts are matched by question text so inserting a prompt cannot shift answers; stale autosaves preserve saved answers to prompts they have not seen. Preview never calls Gemini. When free quota is unavailable, a clearly labelled built-in practice prompt is used. See [free Gemini setup](interview-ai-setup.md).
- **Station review:** finishing or reaching the timer limit saves the transcript and opens its review, including short or empty attempts. The transcript, existing station answer framework and markscheme remain available without AI. Feedback is generated only after an explicit request; its existing 20-word minimum still applies. Camera, accent, movement and eye contact are not assessed. A retry creates a new attempt, preserving the previous transcript in Saved Interviews.
