...
- **Interview:** the active workspace allocates the viewport once, including navigation, preview notices and the room header. The navigation sidebar, transcript and Notes scroll internally; question navigation and call controls stay visible without browser zoom. Compact participant cards use small icon avatars, restrained slate/teal gradients, inset highlights and local camera previews. The transcript sidebar keeps the prompt, answer editor and read-aloud action together, with private Notes alongside. Microphone, camera and finish controls sit beneath the participants; voice and focus controls remain in the header. Small screens stack a compact participant strip above the transcript and keep the control bar in its own bottom row. Existing server timers, draft recovery, autosave and scoring are retained.
